...
impl TableStore {
    /// Creates a new table store backed by an in-memory SageTree.
    pub fn new(info: TableInfo) -> Self {
        Self::with_tree(info, SageTree::new())
    }

    /// Creates a table store backed by the given SageTree.
    ///
    /// Use this when you want a file-backed table: construct a `SageTree`
    /// with a [`FilePager`] and pass it here.
    ...
    /// Flushes the underlying SageTree to disk (no-op for in-memory trees).
    pub fn flush(&self) -> StorageResult<()> {
        self.tree.flush().map_err(StorageError::from)
    }
