...
impl NumaAllocator {
    ...
    fn try_numa_alloc(
        ...
        // Build the node mask bitmask (one bit per node)
        let mask_size = (self.node_count + 63) / 64;
        let mut nodemask: Vec<libc::c_ulong> = vec![0; mask_size];
        nodemask[node_id / 64] |= 1u64 << (node_id % 64);
        ...
        // TODO(numa): Currently we copy from the mmap'd region into a standard
        // AlignedBuffer, which defeats NUMA binding because the new buffer is
        // allocated by the system allocator (first-touch on the copying thread's
        // node). A proper fix requires AlignedBuffer to support mmap-backed
        // storage with munmap on Drop. For now, the first-touch policy during
        // the copy means the buffer lands on the current thread's NUMA node,
        // which is correct when the allocating thread runs on the target node.
    ...
    fn try_interleaved_alloc(&self, size: usize, alignment: usize) -> Option<AlignedBuffer> {
        ...
        // Build mask with all nodes set
        let mask_size = (self.node_count + 63) / 64;
        let mut nodemask: Vec<libc::c_ulong> = vec![0; mask_size];
        for i in 0..self.node_count {
            nodemask[i / 64] |= 1u64 << (i % 64);
        }
