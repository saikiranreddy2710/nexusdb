...
/// for maximum throughput. Falls back to software CRC32 via `crc32fast`.
#[inline]
...
mod tests {
    ...
    #[test]
    fn test_hardware_crc32c() {
        let data = b"Hello, NexusDB!";
        let crc = hardware_crc32c(data);
        assert_ne!(crc, 0); // Just check it produces a non-zero value

        // Same data should produce same hash
        assert_eq!(crc, hardware_crc32c(data));

        // Different data should produce different hash
        assert_ne!(crc, hardware_crc32c(b"Different data"));
    }

    #[test]
    fn test_hardware_crc32c_empty() {
        let crc = hardware_crc32c(b"");
        // Empty data should still produce a valid CRC
        let _ = crc;
    }

    #[test]
    fn test_hardware_crc32c_large() {
