    ...
    fn freeze_memtable(&self) -> KvResult<()> {
        ...
        if !old_memtable.is_empty() {
            ...
            let memtable = Arc::try_unwrap(old_memtable).unwrap_or_else(|_arc| {
                // If other references exist, we need to handle this
                // For now, create a new memtable and copy data
                // This shouldn't happen in normal operation
                warn!("memtable has multiple references during freeze");
                // Return a placeholder - the data is still accessible via the Arc
                MemTable::new(0, 0, 0)
            });
