...
impl MemTable {
    ...
    pub fn put(&self, key: Bytes, value: Bytes) -> KvResult<u64> {
        ...
        // Allocate sequence inside the lock to prevent MVCC gaps
        let seq = self.allocate_sequence();
        // SAFETY: insert requires single-writer, guaranteed by write_lock
    ...
    pub fn delete(&self, key: Bytes) -> KvResult<u64> {
        ...
        let seq = self.allocate_sequence();
        // SAFETY: insert requires single-writer, guaranteed by write_lock
        self.table.insert(InternalKey::delete(key, seq), Bytes::new());
        self.update_sequence_bounds(seq);
        Ok(seq)
    }
    ...
    pub fn apply_batch(&self, batch: &WriteBatch) -> KvResult<u64> {
        ...
        Ok(first_seq.unwrap_or_else(|| self.next_sequence.load(Ordering::Relaxed)))
    }
    ...
    fn check_capacity(&self, additional_bytes: usize) -> KvResult<()> {
        ...
        let limit = self.max_size.saturating_mul(2);
        if current.saturating_add(additional_bytes) > limit {
...
///
/// Holds an `Arc<MemTable>` so that concurrent readers holding a clone
/// of the Arc do not block the freeze operation.
pub struct ImmutableMemTable {
    /// The frozen memtable (shared via Arc to avoid data loss during freeze).
    inner: Arc<MemTable>,
...
impl ImmutableMemTable {
    /// Create an immutable memtable from a shared reference to a mutable one.
...
mod tests {
    ...
    fn test_immutable_memtable() {
        let mt = Arc::new(MemTable::new(1, 1024 * 1024, 1));
        mt.put(Bytes::from("key"), Bytes::from("value")).unwrap();

        let imm = ImmutableMemTable::new(mt);
