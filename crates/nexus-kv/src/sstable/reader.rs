...
///
/// NOTE: Currently pre-reads all entries from all blocks into memory.
/// TODO(perf): Implement lazy block-by-block iteration to reduce memory usage.
pub struct SSTableIterator {
    /// Index of the current block.
    block_idx: usize,
    /// Whether we're at a valid position.
    valid: bool,
    /// Pre-read entries per block: `block_entries[block_idx][entry_idx]`.
    block_entries: Vec<Vec<(Vec<u8>, Vec<u8>)>>,
    /// Current entry index within the current block.
    entry_idx: usize,
}
...
impl SSTableIterator {
    fn new(blocks: Vec<BlockReader>) -> Self {
        ...
        // TODO(perf): Replace with lazy block-by-block iteration to avoid
        // materializing the entire SSTable in memory at once.
    ...
    /// Get the current key (borrows from pre-read entries, no clone).
    pub fn key(&self) -> Option<&[u8]> {
        if self.valid {
            Some(&self.block_entries[self.block_idx][self.entry_idx].0)
        } else {
            None
        }
    }

    /// Get the current value (borrows from pre-read entries, no clone).
    pub fn value(&self) -> Option<&[u8]> {
        if self.valid {
            Some(&self.block_entries[self.block_idx][self.entry_idx].1)
        } else {
            None
        }
    }
    ...
    fn advance_to_valid(&mut self) {
        while self.block_idx < self.block_entries.len() {
            if self.entry_idx < self.block_entries[self.block_idx].len() {
                self.valid = true;
                return;
            }
            // Move to next block
