...
impl Database {
    ...
    ///
    /// If `config.data_dir` is set, the storage engine is disk-backed and
    ...
    pub fn open(config: DatabaseConfig) -> DatabaseResult<Self> {
        let storage = if let Some(ref dir) = config.data_dir {
            let tables_path = std::path::PathBuf::from(dir);
            Arc::new(
                StorageEngine::with_data_dir(&tables_path).map_err(|e| {
                    DatabaseError::Internal(format!("Failed to open storage: {}", e))
                })?,
            )
        } else {
    ...
    pub fn get_or_create_storage(&self, name: &str) -> Arc<StorageEngine> {
        ...
        let storage = if let Some(ref dir) = self.config.data_dir {
            let db_dir = PathBuf::from(dir).join(name);
    ...
    /// Closes the database, flushing all data to disk.
    pub fn close(&self) {
        // Close all sessions
        ...
        // Flush all databases to disk (consolidate + flush pagers + persist catalog)
        let dbs = self.databases.read().unwrap();
        for (name, storage) in dbs.iter() {
            storage.consolidate_all();
